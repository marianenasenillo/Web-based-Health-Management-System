...
// Chart data for deworming records (bar chart by purok)
const purokChartData = computed(() => {
  const purokCounts = {}
  dewormingRecords.value.forEach(record => {
...
const purokExplanation = computed(() => {
  const totalChildren = dewormingRecords.value.length
  ...
  return `This bar chart shows the distribution of ${totalChildren} children receiving deworming medication across different puroks in your barangay. The purok with the highest number of children is ${maxPurok}, indicating the area with the most deworming recipients.`
})
...
const genderExplanation = computed(() => {
  ...
  return `This pie chart illustrates the gender distribution among ${totalChildren} children receiving deworming medication. There are ${maleCount} males (${malePercentage}%) and ${femaleCount} females (${femalePercentage}%) in the program.`
})
...
<template>
  ...
  <div>
    ...
    <div class="text-center mb-4">
      <h4 class="fw-bold">Deworming Program Report (10–19 yrs old)</h4>
      <p>
    ...
    <section class="mb-4">
      ...
      <div class="table-responsive">
        <table class="table table-bordered table-striped">
          ...
          <tbody>
            <tr v-for="record in dewormingRecords" :key="record.id">
              <td>{{ record.purok }}</td>
